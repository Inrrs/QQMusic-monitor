...
async def get_login_qrcode(login_type: str = "QQ"):
    ...
    login_qr = await login.get_qrcode(login_type_enum)
...
# 全局变量用于存储验证码相关信息
_captcha_ticket = None
_captcha_rand_str = None
...
async def send_sms_code(phone: str, captcha_code: str = None, country_code: int = 86):
    ...
    try:
        initialize_qqmusic_session()
        event, message = await login.send_authcode(int(phone), country_code)
        if event == login.PhoneLoginEvents.SEND:
            return {"status": "success", "message": "验证码发送成功"}
        elif event == login.PhoneLoginEvents.CAPTCHA:
            # 需要验证码验证，返回验证码URL
            return {"status": "captcha_required", "message": "需要验证码验证", "captcha_url": message}
        elif event == login.PhoneLoginEvents.FREQUENCY:
            return {"status": "error", "message": "发送频率过高，请稍后重试"}
        else:
            return {"status": "error", "message": message or "验证码发送失败"}
    except Exception as e:
        print(f"发送验证码失败: {e}")
...
async def phone_login(phone: str, auth_code: str, country_code: int = 86):
    ...
    try:
        initialize_qqmusic_session()
        cred = await login.phone_authorize(int(phone), int(auth_code), country_code)
